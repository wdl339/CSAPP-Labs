#/* $begin pipe-all-hcl */
# 韦东良 522031910516
# iaddq add a constant value to a register
# the computations required for iaddq (in SEQ):
#						iaddq V,rB
...
#	PC update		PC <- valP
#
#	The situation is similar in PIPE.
#	Add IIADDQ in instr_valid, need_regids, need_valC because of "iaddq V,rB".
#	Add IIADDQ in srcB(D_rB), dstE(D_rB) because iaddq add a constant value to a register rB.
#	Add IIADDQ in aluA(E_valC), aluB(E_valB) because valE <- ValC + valB.
#	Add IIADDQ in set_cc because the addition should set cc.
#
####################################################################
...
wordsig D_valC 'if_id_curr->valc'     # Incremented PC
...
word d_valA = [
	D_icode in { IJXX, ICALL } : D_valP; # Use incremented PC
