...
stat_t nexti(y64sim_t *sim)
{
    ...
    long_t imm = 0;
    long_t new_val_rsp, val_rsp, addr;
    long_t val_A, val_B, result = 0;
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        addr = compute_alu(A_ADD, get_reg_val(sim->r, regB), imm);
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        addr = compute_alu(A_ADD, get_reg_val(sim->r, regB), imm);
        if (!get_long_val(sim->m, addr, &imm))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addr);
            return STAT_ADR;
        }
      ...
      case I_ALU: /* 6:x regA:regB */
        val_A = get_reg_val(sim->r, regA);
        val_B = get_reg_val(sim->r, regB);
        if (ifun >= 0 && ifun <= 3)
            result = compute_alu(ifun, val_A, val_B);
      ...
      case I_CALL: /* 8:0 imm */
        new_val_rsp = compute_alu(A_SUB, 8, get_reg_val(sim->r, REG_RSP));
        set_reg_val(sim->r, REG_RSP, new_val_rsp);
        if (!set_long_val(sim->m, new_val_rsp, next_pc)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, new_val_rsp);
            return STAT_ADR;
        }
        sim->pc = imm;
        break;
      ...
      case I_RET: /* 9:0 */
        ...
        if (!get_long_val(sim->m, val_rsp, &imm))
        {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, compute_alu(A_ADD, 8, val_rsp));
        sim->pc = imm;
        break;
      ...
      case I_PUSHQ: /* A:0 regA:F */
        val_A = get_reg_val(sim->r, regA);
        val_rsp = get_reg_val(sim->r, REG_RSP);
        new_val_rsp = compute_alu(A_SUB, 8, val_rsp);
        set_reg_val(sim->r, REG_RSP, new_val_rsp);
        if (!set_long_val(sim->m, new_val_rsp, val_A)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, new_val_rsp);
            return STAT_ADR;
        }
        sim->pc = next_pc;
